...
<required_reading>
@~/.gemini/antigravity/gsd-core/references/domain-probes.md
@~/.gemini/antigravity/gsd-core/references/gate-prompts.md
@~/.gemini/antigravity/gsd-core/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "$HOME/.gemini/antigravity/gsd-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@~/.gemini/antigravity/gsd-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
